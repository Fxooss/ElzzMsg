...
socket.on('receive_message', (msg) => { if (currentChat && currentChat.type === 'user' && currentChat.id === msg.from) appendMsg(msg); });
...
function switchTab(tab) {
  ...
  if(tab === 'chats') { document.querySelector('.tab:first-child').classList.add('active'); document.getElementById('chatsView').style.display = 'block'; document.getElementById('statusView').style.display = 'none'; }
  else { document.querySelector('.tab:nth-child(2)').classList.add('active'); document.getElementById('chatsView').style.display = 'none'; document.getElementById('statusView').style.display = 'block'; socket.emit('get_statuses'); }
}
function openMyStatus() { document.getElementById('statusInput').click(); }
document.getElementById('statusInput').onchange = async (e) => { const file = e.target.files[0]; if(!file) return; const fd = new FormData(); fd.append('file', file); const res = await fetch('/upload', { method: 'POST', body: fd }); const d = await res.json(); socket.emit('post_status', { userId: currentUser.id, url: d.url, type: d.type.startsWith('video') ? 'video' : 'image' }); };
...
function openChat(d) { 
  ...
  document.getElementById('groupInfoBtn').style.display = d.type === 'group' ? 'block' : 'none';
...
socket.on('group_messages', (msgs) => { msgs.forEach(m => appendMsg(m, true)); });

function appendMsg(m, isGroup = false) {
  const c = document.getElementById('messagesContainer'); const el = document.createElement('div');
...
function appendBotMsg(text) { const c = document.getElementById('messagesContainer'); const el = document.createElement('div'); el.className = 'message in'; el.innerHTML = `<div class="message-bubble"><span class="message-text" style="white-space:pre-wrap">${text}</span></div>`; c.appendChild(el); c.scrollTop = c.scrollHeight; }

// === ACTIONS ===
function toggleFab() { document.getElementById('fabMenu').classList.toggle('active'); }
function openAddContact() { toggleFab(); document.getElementById('addContactModal').classList.add('active'); document.getElementById('addContactInput').value = ''; }
function closeAddContact() { document.getElementById('addContactModal').classList.remove('active'); }
function addContact() { socket.emit('add_contact', { targetUsername: document.getElementById('addContactInput').value.trim(), userId: currentUser.id }); closeAddContact(); }

function openCreateGroup() { toggleFab(); document.getElementById('createGroupModal').classList.add('active'); document.getElementById('groupNameInput').value = ''; }
function closeCreateGroup() { document.getElementById('createGroupModal').classList.remove('active'); }
function createGroup() { const name = document.getElementById('groupNameInput').value.trim(); if(!name) return; socket.emit('create_group', { name, userId: currentUser.id }); closeCreateGroup(); }

function openGroupInfo() { document.getElementById('groupInfoModal').classList.add('active'); renderMemberList(); }
...
function renderMemberList() { 
  ...
  const group = myGroups.find(g => g.id === currentChat.id); if(!group) return;
  group.members.forEach(mId => {
    const u = myContacts.find(c => c.id === mId) || onlineUsers.find(o => o.id === mId);
    const name = u ? u.username : mId;
    const isAdmin = group.admins.includes(mId);
    const isMeAdmin = group.admins.includes(currentUser.id);
    const div = document.createElement('div'); div.style.cssText = "display:flex;justify-content:space-between;padding:10px;border-bottom:1px solid var(--border)"; 
    div.innerHTML = `<span>${name} ${isAdmin ? '(Admin)' : ''}</span> ${isMeAdmin && !isAdmin ? `<button onclick="promote('${mId}')">Promote</button>` : ''}`; 
    list.appendChild(div); 
  }); 
}
function addMember() { const u = document.getElementById('addMemberInput').value.trim(); if(!u) return; socket.emit('add_member', { groupId: currentChat.id, userId: u }); alert('Added'); }
function promote(id) { socket.emit('promote_admin', { groupId: currentChat.id, userId: id }); alert('Promoted'); }

// === SEND ===
function sendMessage() { 
  const t = document.getElementById('messageInput').value.trim(); 
  if (!t || !currentChat) return; 
  if (currentChat.type === 'user') socket.emit('send_message', { from: currentUser.username, to: currentChat.id, text: t });
  else socket.emit('send_group_message', { from: currentUser.id, to: currentChat.id, text: t });
...
// === UTILS ===
function tagUser(username) { const input = document.getElementById('messageInput'); input.value += `@${username} `; input.focus(); }
